...
class td_block_5 extends td_block {
    ...
    function render($atts, $content = null){
        ...
        if($this->get_att('installed_post_types') == 'product') $products_class = 'products';
        else $products_class = null;

        $buffy = ''; //output buffer

        $buffy .= '<div class="'. $this->get_block_classes() . ' '. $products_class .' td-column-' . $td_column_number . ' td_block_padding" ' . $this->get_block_html_atts() . '>';
        ...
        if (!empty($posts)) {
            foreach ($posts as $post) {
                if($post->post_type == 'product') {
                    $td_module_3 = new td_module_product($post);
                }else{
